...
# Tạo user container theo chuẩn Pterodactyl
...
RUN          apt update \
             && apt upgrade -y \
             && apt install -y gcc g++ libgcc-12-dev libc++-dev gdb libc6 git wget curl tar zip unzip binutils xz-utils \
             ...
             locales ffmpeg gnupg2 apt-transport-https software-properties-common ca-certificates liblua5.3-0 \
             libz3-dev libzadc4 rapidjson-dev tzdata libevent-dev libzip4 libprotobuf32 libfluidsynth3 procps \
             libstdc++6 tini

# Cài libicu66 (cho các app cần) và libssl1.1 >= 1.1.1f để hỗ trợ Bedrock Server
RUN if [ "$(uname -m)" = "x86_64" ]; then \
      ...
      rm libicu66_66.1-2ubuntu2.1_amd64.deb libssl1.1_1.1.1f-1ubuntu2.17_amd64.deb; \
    else \
...
# Thư mục làm việc chuẩn cho Pterodactyl
WORKDIR /home/container

# Xử lý tín hiệu dừng
STOPSIGNAL SIGINT

# Copy entrypoint (theo chuẩn yolks)
COPY --chown=container:container ./entrypoint.sh /entrypoint.sh
RUN chmod +x /entrypoint.sh

# Mặc định chạy entrypoint
